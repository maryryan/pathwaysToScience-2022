...
- Used with 2 categorical variables

  - i.e., types of issues and severity of issue
  
- One category goes on one axis

- Percentage goes on other axis

- Total bar height will always go up to 100%
  
  - Bar will be divided into percentages based on 2nd categorical variable
...
mtcars %>% 
  ggplot(aes(y=mpg, x=as.factor(cyl)))
```
